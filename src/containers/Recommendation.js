...
export default class Blog extends Component {
    componentDidMount() {
        axios.get(`/api/v${API_INFO.VERSION}/recommendations/`, {
            params: { page: this.state.page }
        }).then(({ data: recommendations }) => {
            this.setState({
                recommendations,
                more: recommendations.length !== 0 && recommendations.length % RECOMMENDATION.RECOMMENDATIONS_PER_PAGE === 0
            });
        }).catch(err => {
            console.error(err);
        });
    ...
    render() {
        return (
            <div className="Recommendation padding-all--md">
                ...
                {!!this.state.recommendations.length && <RecommendationArchive recommendations={this.state.recommendations} />}
