...
export default class Projects extends Component {
   componentDidMount() {
      axios
         .get(`/api/v${API_INFO.VERSION}/project/posts/`, {
         ...
         .then(({ data: projects = [] }) => {
            this.setState({
               ...
               more:
                  projects.length !== 0 &&
                  projects.length % PROJECT.PROJECTS_PER_PAGE === 0
            });
         })
         .catch(err => {
            console.error(err);
      ...
      axios
         ...
         .then(({ data: projects }) => {
            this.setState({
               ...
               more:
                  projects.length !== 0 &&
                  projects.length % PROJECT.POSTS_PER_PAGE === 0
            });
         })
         .catch(err => {
            console.error(err);
      ...
      return (
         <div className="Projects padding-all--md">
            ...
            {!!this.state.projects.length && (
               <ProjectArchive projects={this.state.projects} />
            )}
            {!!this.state.projects.length && (
               <Button.Light
